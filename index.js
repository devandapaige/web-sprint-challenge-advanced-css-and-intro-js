...
console.log(artists[8].name); //to test that 8 is the index//
...
function get20s(array, birthmin, deathmax) {
  let artists20 = [];
  for (let i = 0; i < artists.length; i++) {
    if (artists[i].years.split(" - ")[0] >= birthmin) {
      if (artists[i].years.split(" - ")[1] < deathmax) {
...
//had to comment out since a blank array index would not allow my other functions to work below.
...
function addArtist(array) {
  array.push({
    ...
    years: "1992 - alive",
    genre: "Full-Stack",
    nationality: "Cheese Head",
    bio: "Don't overthink it, and just keep moving.",
    wikipedia: "none",
    paintings: 36,
  });
  return array;
